...
def simulate_insurance_company(n_0, a_0, lamb, c, v, m, weib_a, T):
    ...
    while t < T and a >= 0:
        ...
        claims = np.random.poisson(lamb, n)
        claim_costs = np.random.weibull(weib_a, n)
        ...
        retention_rates = np.random.exponential(m, n)
...
def plot_ex(variable, value, amounts):
...
def execute(std_threshold, n_0, a_0, lamb, c, v, m, weib_a, T, varible_name, variable_value):
    ...
    plot_ex(varible_name, variable_value, amounts)
